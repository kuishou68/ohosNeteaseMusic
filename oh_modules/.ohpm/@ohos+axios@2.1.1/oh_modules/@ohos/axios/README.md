...
- T: 是请求体的类型。当你发送一个 POST 请求时，你可能会发送一个 JSON 对象。T 就是这个 JSON 对象的类型。默认情况下，T 是 any，这意味着你可以发送任何类型的数据。
...
```
### 指定自定义证书路径
将证书放置在resoueces rawfile目录下，目前仅支持后缀名为.pem的文本格式证书。
```javascript
  let context: Context = getContext(this);
    context.resourceManager.getRawFileContent("ca.pem", (error, value: Uint8Array) => {
      if (error != null) {
        console.log("error is " + error);
      } else {
        let filesDir: string = context.filesDir
        let filePath = filesDir + "/ca.pem";
...
- 上传文件必须设置onUploadProgress方法
- 请求的表单数据值为string类型

##### 当上传的内容为ArrayBuffer时，必须指定文件名，用法如下
